...
def extract_participants(state: State) -> State:
    ...
    #Esta línea es una "List Comprehension" (comprensión de listas) en Python. Es una forma muy
    #  común y potente de procesar listas en una sola línea.
    # 1. El LLM te devuelve un texto como este: " Juan García, María López, Carlos Ruiz ". El 
    # método .split(',') corta ese texto cada vez que encuentra una coma y lo convierte en una lista:
    # 2. El método .strip() elimina los espacios sobrantes al principio y al final.
...
# Minuta" es simplemente un término formal para referirse al Acta de la Reunión 
# (en inglés se llama Meeting Minutes)
